...
public class BitSetStrToBitSetConverter {
	...
	public static void main(String[] args) throws Exception {
		...
		CanditateGeneratorStageKGT2 candStageKGT2 = new CanditateGeneratorStageKGT2(4, passedCandidates);
		HashMap<BitSet, ArrayList<BitSet>> cdParttion = candStageKGT2.getCandidatePartition();
			...
			for (int i = 0; i < bsList.size()-1; i++) {
				...
				for (int j = i + 1; j < bsList.size(); j++) {
					...
					if (allOneBS1.cardinality() == 4) {
						...
						System.out.println("New Candidate::"+ newCand.toString());
						newCand.or(bsList.get(j));
						System.out.println("New Candidate to test::"+newCand.toString());
						if(candStageKGT2.isPresent(newCand)){
							System.out.println("Candidate Present in Base Feed::"+newCand.toString());
						}
